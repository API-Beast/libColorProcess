...
namespace Palette
{
	...
	EntryT map_color_relative(const ColorPalette<EntryT>& pal, EntryT input, StatsFn analysis_function, const std::pair<StatsT, StatsT>& paletteStats, const std::pair<StatsT, StatsT>& sourceStats)
	{
		assert(pal.size() > 0);
	...
	EntryT map_color_absolute(const ColorPalette<EntryT>& pal, EntryT input) 
	{
		...
		for(auto& entry : pal)
		{
			float distance = (input - entry).squareLength();
			if(distance < lowest_distance)
			{
				best_entry = &entry;
				lowest_distance = distance;
	...
	EntryT map_color_relative_multisample(const ColorPalette<EntryT>& pal, EntryT input, StatsFn analysis_function, const std::pair<StatsT, StatsT>& paletteStats, const std::pair<StatsT, StatsT>& sourceStats) 
	{
		...
		constexpr auto NumSamples = std::pow(2, NumAxis);
